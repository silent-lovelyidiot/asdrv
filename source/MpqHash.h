#ifndef __MPQ_HASH_H__
#define __MPQ_HASH_H__

#include <ntdef.h>

#define MPQ_HASH_TABLE_SIZE 0x500
//constexpr ULONG HashTable[0x300] = {
//  //0x000
...
constexpr ULONG HashSeed1 = 0x7FED7FED;
constexpr ULONG HashSeed2 = 0xEEEEEEEE;
...
template<typename T> constexpr ULONG ToUpper(T Ch) { return ToUpper<CHAR>(Ch); }
template<typename T> constexpr ULONG ToUpper(T* Ch) { return ToUpper(*(CHAR*)Ch); }

template<> constexpr ULONG ToUpper(CHAR Ch) {
  return 'a' <= Ch && Ch <= 'z' ?
    Ch + ('A' - 'a') : Ch;
}

template<> constexpr ULONG ToUpper(WCHAR Ch) {
  return L'a' <= Ch && Ch <= L'z' ?
    Ch + (L'A' - L'a') : Ch;
}

// Data must be a string encoded with utf8.
ULONG MpqHashData(CONST VOID* Data, ULONG Size, HashType Type,
  PCULONG Table,
  ULONG Seed1 = HashSeed1, ULONG Seed2 = HashSeed2);
...
// utf8 string
constexpr ULONG operator "" _hashA(const char* Str, size_t Size) {
  return HashInternal(Str, Size, HashTypeA);
}

// utf8 string
constexpr ULONG operator "" _hashB(const char* Str, size_t Size) {
  return HashInternal(Str, Size, HashTypeB);
}

constexpr SIZE_T operator "" _L(const wchar_t* Str, size_t Size) {
  Str;
  return Size;
}

constexpr SIZE_T operator "" _S(const wchar_t* Str, size_t Size) {
  Str;
  return Size * sizeof(__wchar_t);
}

#define HASHU(s, t) u8##s##t
#define HASHS(s, t) L##s##t
#define HASHL(s, t) L##s##t

#define HashO(s)    HASHU(s, _hashO)
#define HashA(s)    HASHU(s, _hashA)
#define HashB(s)    HASHU(s, _hashB)
#define HashS(s)    HASHS(s, _S)
#define HashL(s)    HASHL(s, _L)

#if DBG
#define ConstexprHashO(r, s)    enum : ULONG {r = HashO(s) };//constexpr ULONG r = HashO(s);
#define ConstexprHashA(r, s)    enum : ULONG {r = HashA(s) };//constexpr ULONG r = HashA(s);
#define ConstexprHashB(r, s)    enum : ULONG {r = HashB(s) };//constexpr ULONG r = HashB(s);
#define ConstexprSize(r, s)     enum : ULONG {r = HashS(s) };//constexpr ULONG r = HashS(s);
#define ConstexprULL(r)      constexpr ULONGLONG r##ULL = ((ULONGLONG)(r##A) << 32) | r##B;
#else
#define ConstexprHashO(r, s)    constexpr ULONG r = (ULONG)HashO(s);
#define ConstexprHashA(r, s)    constexpr ULONG r = (ULONG)HashA(s);
#define ConstexprHashB(r, s)    constexpr ULONG r = (ULONG)HashB(s);
#define ConstexprSize(r, s)     constexpr ULONG r = (ULONG)HashS(s);
#define ConstexprULL(r)      constexpr ULONGLONG r##ULL = ((ULONGLONG)(r##A) << 32) | r##B;
#endif

#define ConstexprHashOABS(r, s)  ConstexprHashO(r##O, s);ConstexprHashA(r##A, s);ConstexprHashB(r##B, s);ConstexprSize(r##S, s);ConstexprULL(r)
